...
DOM.imageModal.style.display='none';DOM.modalClose.addEventListener('click',hideModal);DOM.modalImage.addEventListener('click',toggleImageZoom);DOM.downloadBtn.addEventListener('click',downloadImage);DOM.zoomBtn.addEventListener('click',toggleImageZoom);DOM.imageModal.addEventListener('click',function(e){if(e.target===DOM.imageModal){hideModal()}});document.addEventListener('keydown',function(e){if(e.key==='Escape'&&DOM.imageModal.classList.contains('modal--active')){hideModal()}});console.log('Modal inicializado correctamente')},show:function(imageSrc,imageTitle=''){if(!validateDOM()){console.error('Modal: No se puede mostrar - elementos del DOM no disponibles');return}
